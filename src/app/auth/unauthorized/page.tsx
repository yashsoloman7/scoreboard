...
// src/app/auth/unauthorized/page.tsx - Role Authorization Pending Notice & Live Verification Portal

import React, { useState, useEffect, useCallback } from 'react';
import { useAuth } from '@/lib/auth/AuthContext';
import { ShieldAlert, RefreshCw, LogOut, Mail, CheckCircle2, ArrowRight, ShieldCheck, AlertTriangle } from 'lucide-react';
...
export default function UnauthorizedPage() {
  ...
  const handleCheckAuthorization = useCallback(async () => {
    try {
      ...
      if (destination && role !== 'unauthorized') {
        ...
        setStatusMessage(`Status checked at ${time}: Role is still marked as "Unauthorized". Once your Administrator grants your Judge/Admin role, click Check Authorization again.`);
      }
    } catch (err: unknown) {
      setStatusType('error');
  ...
  }, [refreshProfile, user?.role, router]);

  // Auto-redirect if user already has an authorized role
  useEffect(() => {
    if (!isLoading && user?.role && user.role !== 'unauthorized') {
      ...
      <div className="w-full max-w-lg bg-slate-900 border border-slate-800 rounded-3xl shadow-2xl p-6 sm:p-8 backdrop-blur-xl text-center">
        ...
        <div className="p-4 rounded-2xl bg-slate-800/50 border border-slate-700/60 text-left text-xs text-slate-300 space-y-2 mb-6">
          ...
          <ul className="list-disc pl-5 space-y-1.5 text-slate-400">
            <li>Notify your Competition Super Admin or Chief Scrutineer.</li>
